...
// TODO: Import lodash, $setPath, $watchAll etc?
...
app.component('vTable', {
	...
	methods: {
		...
		*/
		refresh() {
			$debug('refresh', this.reloadCount, this.url, this.endpointSearch);
			if (this.state == 'loading') return; // Already refreshing
			if (!this.url) return $debug('Skipping refresh due to falsy URL'); // No URL available yet, URL is probably dynamic - do nothing
			...
			return Promise.resolve()
				...
				.then(req => { $debug('AxiosRequest', req); return req })
				.then(req => {
				...
				.then(()=> $debug('Row data', {rows: this.rows, rowCount: this.rowCount}))
		...
		*/
		setSort(columnId, behaviour = 'set') {
			...
			$debug('setSort', columnId, behaviour, this.endpointSort, this.endpointSortAsc);
