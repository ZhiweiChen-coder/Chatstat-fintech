...
// TODO: Import $components, $digest, $files, $http, $toast and $watchAll from packages?
//import Files from '@doop/files';
//const $files = Files();
		...
		*/
		refresh() {
			return Promise.resolve()
				...
				.then(()=> {
					if (!this.email && !this.user?.email && !this.userResolved?.email) return; // Not enough data - do nothing
					this.imageUrl = 'https://gravatar.com/avatar/'
						+ MD5.hash(this.email ? this.email : this.userResolved.email ? this.userResolved.email : this.user.email).toString()
						+ '?'
						+ `size=${this.size}&`
						+ `d=${this.fallback}`
					throw 'DONE';
				...
				.then((imageUrl)=> {
					this.imageUrl = imageUrl || this.failIcon;
					})
				// }}}
				// Catch (skip 'DONE') {{{
				.catch(e => {
...
<template>
	<div class="user-avatar" :class="editable && 'editable'" :style="{ width: `${size}px`, height: `${size}px` }" v-tooltip="tooltip" @click="upload">
